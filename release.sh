...
while test $# -gt 0; do
    case "$1" in
        -h|--help)
            echo "release - Release Pollfish Max Android Adapter"
            echo " "
...
if [ "$publish_only" = false ] ; then
    ...
    # Build Pollfish Max Adapter 

    if ./gradlew :pollfish-max:build; then
    ...
    if ./gradlew :pollfish-max:packageDistributions; then
...
if [ "$no_publish" = false ] ; then
    ...
    # Upload Pollfish Max Adapter aar to Sonatype repository

    if ./gradlew :pollfish-max:publishAllPublicationsToSonatypeRepository; then
    ...
    for entry in pollfish-max/build/dist/public/*
    do
        file_name="${entry##*/}"
        slashed_entry="${entry// /\\ }"
        ...
        eval "gsutil cp ${slashed_entry} gs://pollfish_production/sdk/AppLovin/"
        if [ $? -eq 0 ]; then
        ...
        eval "gsutil acl ch -u AllUsers:R gs://pollfish_production/sdk/AppLovin/${slashed_file_name}"
        if [ $? -eq 0 ]; then
            echo "${file_name} is now public"
            echo "URL: https://storage.googleapis.com/pollfish_production/sdk/AppLovin/${file_name// /%20}"
        else
